...
function startAutoMode() {
  ...
  console.log(`[${new Date().toISOString()}] Starting auto mode (${config.intervalHours}h intervals)`);
  ...
  console.log(`[${new Date().toISOString()}] Next execution in ${config.intervalHours} hours`);
}
...
function startManualMode(options) {
  ...
  console.log(`[${new Date().toISOString()}] Starting manual mode: ${count} calls starting at ${fromTime}, ${intervalHours}h intervals`);
  ...
  for (let i = 0; i < count; i++) {
    ...
    console.log(`[${new Date().toISOString()}] Execution ${i + 1}/${count} scheduled for: ${scheduledTime.toLocaleString()}`);
    
    const timeout = setTimeout(() => {
      console.log(`[${new Date().toISOString()}] Executing scheduled task ${i + 1}/${count}`);
      executeScheduledTask();
    }, delay);
...
async function executeScheduledTask() {
  try {
    console.log(`[${new Date().toISOString()}] Executing scheduled burst sequence...`);
    const successCount = await executeBurstSequence();
    console.log(`[${new Date().toISOString()}] Scheduled execution completed with ${successCount} successful attempts`);
  } catch (error) {
    console.error(`[${new Date().toISOString()}] Scheduled execution failed:`, error.message);
  }
}
...
function startDaemon(options = {}) {
  ...
  console.log(`[${new Date().toISOString()}] Daemon started successfully`);
}
